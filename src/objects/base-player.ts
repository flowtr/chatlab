...
export class CharacterController {
    ...
    private _camera: ArcRotateCamera | VRDeviceOrientationArcRotateCamera;
    ...
    private _minSlopeLimit = 30;
    private _maxSlopeLimit = 45;
    ...
    private _anims: _AnimData[] = [
        ...
        this._turnRightFast,
        this._strafeLeft,
        this._strafeLeftFast,
        this._strafeRight,
        this._strafeRightFast,
        this._slideBack,
    ];

    // move keys
    private _walkKey = "w";
    private _walkBackKey = "s";
    private _turnLeftKey = "";
    private _turnRightKey = "";
    private _strafeLeftKey = "a";
    private _strafeRightKey = "d";
    private _jumpKey = " ";

    private _elasticCamera = true;
     ...
     */
    public setAnimationGroups(agMap: Record<string, AnimationGroup>) {
        this._isAG = true;
    ...
    public setAnimationRanges(arMap: Record<string, AnimationRange>) {
        this._isAG = false;
        let arData: string | AnimationRange;
        for (const anim of this._anims) {
    ...
    public enableBlending(n: number) {
        if (this._isAG)
            for (const anim of this._anims) {
                if (anim._exist) {
                    const ar: AnimationGroup = anim._ag;
                    for (const ta of ar.targetedAnimations) {
                        ta.animation.enableBlending = true;
                        ta.animation.blendingSpeed = n;
                    }
                }
            }
        else this._skeleton.enableBlending(n);
    }
    ...
    public setWalkAnim(rangeName: string | AnimationGroup, rate: number, loop) {
        this._setAnim(this._walk, rangeName, rate, loop);
    }
    public setRunAnim(rangeName: string | AnimationGroup, rate: number, loop) {
    ...
    public setIdleAnim(rangeName: string | AnimationGroup, rate: number, loop) {
    ...
    private checkAGs(agMap: Record<string, AnimationGroup>) {
        for (const anim of this._anims)
            if (agMap[anim._name] != null) {
                anim._ag = agMap[anim._name];
                anim._exist = true;
    ...
    private _fallFrameCountMin = 50;
    private _fallFrameCount = 0;

    private _inFreeFall = false;
    ...
    private _moveAVandCamera() {
        ...
        } else if (!this._inFreeFall) anim = this._doIdle(dt);
    ...
    private _jumpStartPosY = 0;
    //for how long the AV has been in the jump
    private _jumpTime = 0;
    private _doJump(dt: number): _AnimData {
        let anim: _AnimData = null;
        anim = this._runJump;
        if (this._jumpTime === 0) this._jumpStartPosY = this._avatar.position.y;
        ...
        if (this._wasRunning || this._wasWalking) {
            if (this._wasRunning) forwardDist = this._runSpeed * dt;
            else if (this._wasWalking) forwardDist = this._walkSpeed * dt;
    ...
    private _doMove(dt: number): _AnimData {
            ...
            if (this.mode == 1) {
                ...
                if (this._act._turnLeft)
                    if (this._act._walk) {
                        ...
                        anim =
                            this._sign > 0 ? this._turnRight : this._turnLeft;
                else if (this._act._walk) a = -this._sign;
                ...
                this._avatar.rotation.y =
                    this._avatar.rotation.y + turnAngle * a;
            } else {
                let a = 1;
                if (this._act._turnLeft) {
        ...
        if (this.mode != 1)
            if (this._noRot)
                switch (true) {
                    ...
                    case this._act._walk:
                        this._avatar.rotation.y =
                            this._av2cam - this._camera.alpha;
                        break;
                    case this._act._walkback:
                        this._avatar.rotation.y =
                            this._av2cam - this._camera.alpha + Math.PI;
                        break;
            ...
            else this._avatar.rotation.y = this._av2cam - this._camera.alpha;
        else {
        }
        ...
        if (moving)
            if (this._moveVector.length() > 0.001) {
                ...
                if (this._avatar.position.y > this._avStartPos.y) {
                    ...
                    if (_sl >= this._sl2)
                        if (this._stepOffset > 0) {
                            ...
                            if (this._vMoveTot > this._stepOffset) {
                                ...
                                this._vMoveTot = 0;
                                this._avatar.position.copyFrom(
                                    this._vMovStartPos
                                );
                                this._endFreeFall();
                            }
                    ...
                    if (
                        !this._areVectorsEqual(actDisp, this._moveVector, 0.001)
                    )
                        if (this._verticalSlope(actDisp) <= this._sl)
                            //AV is on slope
    ...
    private _idleFallTime = 0;
    private _doIdle(dt: number): _AnimData {
        if (this._grounded) return this._idle;
    ...
    private _snapCamera() {
        ...
        const pi: PickingInfo = this._scene.pickWithRay(
            this._ray,
            (mesh) => {
        ...
        if (pi.hit)
            if (this._camera.checkCollisions) {
                ...
                const newPos: Vector3 = this._camera.target
                    .subtract(pi.pickedPoint)
                    .normalize()
                    .scale(this._cameraSkin);
                pi.pickedPoint.addToRef(newPos, this._camera.position);
            } else {
                const nr: number = pi.pickedPoint
                    .subtract(this._camera.target)
                    .length();
    ...
    public strafeLeftFast(b) {
        this._act._stepLeft = b;
        this._act._speedMod = b;
    }
    public strafeRight(b) {
        this._act._stepRight = b;
    }
    public strafeRightFast(b) {
        this._act._stepRight = b;
        this._act._speedMod = b;
    }
    public jump() {
    ...
    constructor(
        ...
        scene: Scene,
        agMap?: Record<string, AnimationGroup>,
        faceForward = false
    ) {
        this._avatar = avatar;
        ...
        if (this._isAG || this._skeleton !== null) this._hasAnims = true;
...
class _Action {
    public _walk = false;
    public _walkback = false;
    // speed modifier - changes speed of movement
    public _speedMod = false;
    public _turnRight = false;
    public _turnLeft = false;
    public _stepRight = false;
    public _stepLeft = false;
    public _jump = false;
